...
public class DeviceManagerAdministrator_Page extends utility 
{
	public WebDriver wd;
      
	public DeviceManagerAdministrator_Page(WebDriver wd) 
       {
    	   this.wd = wd;
    ...
    @FindBy (xpath = "//input[@id='ActiveX Installer (AxInstSV)']")
    ...
    */
    ...
    public void applicationcommand() 
    {
    	...
    	if(Clickadministration.isDisplayed()==false)	
  	  {
  		  clickGrp.click();
  		  clickNode.click();
  		  btnExpandMenu.click();
        ...
        if(btnnewEnvVariable.isDisplayed()==true)
        {
        	a.moveToElement(btnnewEnvVariable).release().build().perform();
        	btnnewEnvVariable.click();
        	a.moveToElement(btnapplyEnvVariable).release().build().perform();
        }
        txtVariablename.sendKeys("CMD_Auto");
        txtvalue.sendKeys("C:\\Windows\\System32\\cmd.exe");
        btnapplyEnvVariable.click();
        
        utility.isVisible(lblapplyEnvVariable, wd, 10);
    ...
    public void performancemgmt()
	{
		...
		Select dropdown = new Select(ddBrowserInstall);
    ...
    public void remoteagent() 
    {
		...
		txtserverIP.sendKeys("");
		txtportno.sendKeys("");
		txtHeartbeatint.sendKeys("");
		...
		btnapplyuserMGMT.click(); //Request for settings update has been processed
		...
		btnapplyUSBDevicemanager.click(); //USB Device Manager Settings applied successfully
